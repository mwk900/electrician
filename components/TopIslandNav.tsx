...
export default function TopIslandNav() {
  const [emergencyOpen, setEmergencyOpen] = useState(false);
  const emergencyPanelRef = useRef<HTMLDivElement | null>(null);
  ...
  useEffect(() => {
    ...
    function onPointerDown(event: PointerEvent) {
      ...
      if (emergencyPanelRef.current?.contains(target)) return;
      if (emergencyButtonRef.current?.contains(target)) return;
